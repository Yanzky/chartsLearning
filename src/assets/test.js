...
setInterval(function() {var num = Math.random() * 180; var numPi = Math.floor(num - 89) * (pi/180);// Get value
...
// Arc Transition
  ...
  hold = cur_color; cur_color = new_color; new_color = hold;}, 1500); // Repeat every 1.5s
function arcTween(transition, newAngle) {transition.attrTween("d", function(d) {var interpolate = d3.interpolate(d.endAngle, newAngle);
